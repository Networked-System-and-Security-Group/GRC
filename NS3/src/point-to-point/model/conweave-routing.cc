...
void ConWeaveRouting::SendReply(Ptr<Packet> p, CustomHeader &ch, uint32_t flagReply,
    ...
    uint32_t dstToRId = Settings::flowInfos[flow_id].srcTor;;
...
void ConWeaveRouting::RouteInput(Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (ch.l3Prot == 0x11){
        ...
        srcToRId = Settings::flowInfos[flow_id].srcTor;
        dstToRId = Settings::flowInfos[flow_id].dstTor;
    }
    else {
        flow_id = Settings::get_flowid(p);
        srcToRId = Settings::flowInfos[flow_id].dstTor;
        dstToRId = Settings::flowInfos[flow_id].srcTor;
    }
    if (Debug_log){
        if (ch.l3Prot == 0x11){
    ...
    if (m_isToR) {  // ToR switch
         ...
         ***************************/
            ...
            if (foundConWeaveReplyTag) {  // Received REPLY
                //检查接受的交换机是否是数据发送的交换机
                if(Control_info_log){
                    uint32_t show_flow_id = Settings::get_flowid(p);
                    uint32_t Data_src_ToR = Settings::flowInfos[show_flow_id].srcTor;
