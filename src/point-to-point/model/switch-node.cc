    ...
    static TypeId tid =
        TypeId("ns3::SwitchNode")
            ...
            .AddAttribute("AckHighPrio", "Set high priority for ACK/NACK or not", UintegerValue(0),
                          ...
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("PfcEnabled", "Enable PFC.", BooleanValue(true),
                          MakeBooleanAccessor(&SwitchNode::m_pfcEnabled), MakeBooleanChecker());
    return tid;
}
...
bool SwitchNode::SwitchReceiveFromDevice(Ptr<NetDevice> device, Ptr<Packet> packet,
    ...
    if (ch.l3Prot == 0x11)  // XXX RDMA traffic on UDP
    {
        ...
        if (Settings::flowInfos[flow_id].srcTor == 0xFFFFFFFF) { //每条流经过的第一个交换机是源Tor交换机
            auto &flowItem = Settings::flowInfos[flow_id];
            flowItem.srcTor = m_id;
            const auto& dstTorCandidates = Settings::hostId2ToRlist[flowItem.dst];
            flowItem.dstTor = dstTorCandidates[std::rand() % dstTorCandidates.size()];
        }
        Settings::flowInfos[flow_id].record_switch_node(m_id);
...
int SwitchNode::GetOutDev(Ptr<Packet> p, CustomHeader &ch) {
    ...
    } else if (Settings::nodeInfos[m_id].node_type == NodeInfo::NodeType::DCI_SWITCH) {
        //如果是DCI交换机，如果是目的地在同一AS内，直接查找路由表。否则，查找WAN路由
        ...
        if (m_mmu->CheckEgressAdmission(outDev, qIndex,
            ...
            if (ch.l3Prot == 0x11) {
                printf("An UDP packet dropped because egress admission check false: Node:%u, Flow:%u, Seq=%u\n", 
                    ...
                    Settings::get_flowid(p),
                    ch.udp.seq);
                //timestamp_ns,switch_id,related_node,flow_id,seq_num,type
                fprintf(logfile::drop_log, "%lu,%u,%u,%u,%u,%u\n", 
                    Simulator::Now().GetNanoSeconds(),
                    this->m_id,
