...
def _run_baseline(args, skip_build: bool):
    ...
    return _load_flow_metrics(flow_output), str(out_dir)
...
def _write_reports(report_dir: Path, results: list[Result], baseline: dict):
    ...
    best_tail = min(results, key=lambda r: (r.p99_fct, r.avg_fct, -r.completion_rate))

    lines = []
    lines.append("# WAN RED/FEC 60 组宽间隔联调结果")
    lines.append("")
    lines.append(f"- baseline avg FCT: {baseline['avg_fct']:.6f}s")
    lines.append(f"- baseline p99 FCT: {baseline['p99_fct']:.6f}s")
    lines.append(f"- best avg: {best.label}")
    lines.append(f"- best tail: {best_tail.label}")
    lines.append("")
    ...
    print(f"best avg: {best.label} avg={best.avg_fct:.6f}s p99={best.p99_fct:.6f}s")
    print(f"best tail: {best_tail.label} avg={best_tail.avg_fct:.6f}s p99={best_tail.p99_fct:.6f}s")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--flow", default="w-dynamic-150-200-200")
    ...
    red_configs = [
        (65536, 524288, 0.01, 0.0005),
        (65536, 1048576, 0.02, 0.0005),
        (131072, 1048576, 0.02, 0.0010),
        (131072, 2097152, 0.05, 0.0010),
        (262144, 1048576, 0.05, 0.0010),
        (262144, 2097152, 0.05, 0.0020),
        (524288, 2097152, 0.05, 0.0020),
        (524288, 4194304, 0.10, 0.0020),
        (1048576, 2097152, 0.10, 0.0050),
        (1048576, 4194304, 0.10, 0.0050),
        (2097152, 4194304, 0.20, 0.0100),
        (262144, 4194304, 0.20, 0.0100),
    ]
    fec_values = [0, 2, 4, 8, 12]
    combos = []
    for idx, (kmin, kmax, pmax, wq) in enumerate(red_configs, start=1):
        for fec_n in fec_values:
            combos.append(
                Combo(
                    label=f"wide-r{idx}-fec{fec_n}",
                    red_kmin=kmin,
                    red_kmax=kmax,
                    red_pmax=pmax,
                    red_wq=wq,
