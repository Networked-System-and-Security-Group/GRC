...
# 绘制inter flow公平性的脚本，DC0,DC2,DC5 --> DC1
...
results = get_avg_interfct_groupby_as('10-13')
for df in results:
    for _,row in df.iterrows():
        pair = (row['src_as'], row['dst_as'])
        fct_dict.setdefault(pair, []).append(row['fct_slowdown'])

print(fct_dict)

data = {}

for pair, fct in fct_dict.items():
    data[f'GSCC-Inter DC{int(pair[0])} to DC{int(pair[1])}'] = ([30,40,50,60], fct)

#data = {
#    'GSCC-Inter':    ([150,180,210,240,270], avg_fct_inter.tolist()),
#}
...
plot_auto_lines(
    ...
    filename="inter_flow_fairness_analysis.pdf",
    xticks=[30,40,50,60],
    xlim=(25,65)
)
quit()