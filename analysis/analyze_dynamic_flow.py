...
if __name__ == "__main__":
    ana = get_analyser(36)  # 替换为实际实验ID
    ana.diagnose_slow_flow()
    print()
    ana.show_drop()
    
    # 原有函数调用
    plot_as_rate_custom(ana, src_as=4, dst_as=3)
    plot_link_utilization_custom(ana, src_id=225, dst_id=226)
    plot_rtt_custom(ana, src_as=4, dst_as=3)
    ...
    plot_buffer_custom(ana, switch_id=222, egress=True)
    plot_pfc_custom(ana, node_id=201)