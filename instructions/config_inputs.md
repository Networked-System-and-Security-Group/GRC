...
- `run.py --tcp_queue_index {1,3}` writes `TCP_QUEUE_INDEX` into `config.txt`.
- `1` keeps TCP on isolated q1; `3` places TCP on q3 so it shares the normal RDMA PG3 queue.
...
- For TCP flows, `pg` is currently treated as an input field for compatibility; queue selection is controlled by `TCP_QUEUE_INDEX`, so recommended `pg` is `1` in isolated mode and the RDMA flow should use `pg=3` when testing q3 sharing.
- FEC does not change the flow file format in the current plan.
- If FEC is enabled, `ReadFlowInput()` must bind the default `FEC_PARITY_PKTS` from `config.txt` to each RDMA flow.
