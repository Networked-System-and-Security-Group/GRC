...
- `--tcp_queue_index`（默认 `1`）：TCP 在 host 和交换机侧使用的队列编号，只允许 `1` 或 `3`
	- `1`：TCP 使用独立 q1，保持当前 TCP/RDMA 队列隔离行为；
	- `3`：TCP 使用 q3，与 RDMA flow 的 PG3 共用普通优先级队列和 shared-pool 逻辑；
	- `run.py` 会将该参数写入 `TCP_QUEUE_INDEX`，复用已有 `config.txt` 时也会更新该键。
...
- 网卡：`src/point-to-point/model/qbb-net-device.cc/.h`
	- 实现 `QbbNetDevice::Send()`：给 TCP/IP 栈提供 NetDevice 发送入口，并把 TCP 包入队到 `TCP_QUEUE_INDEX` 指定的 egress 队列
	- `Receive()` TCP 分流：PPP+IPv4+TCP 直接走 `m_rxCallback` 上交协议栈；其他包保持原有 RDMA/PFC 逻辑
	- Host 侧调度：在 `RdmaEgressQueue::GetNextQindex()` 里按 `TCP_QUEUE_INDEX` 检查 TCP 队列，并在 `DequeueAndTransmit()` 中支持从 `BEgressQueue` 发包
- 交换机/MMU：TCP 包按 `TCP_QUEUE_INDEX` 分类；只有 q1 隔离模式启用 160 MiB TCP 特殊 ingress 阈值，q3 共用模式走 RDMA PG3 的普通 shared-pool/headroom 逻辑

> 备注：以上修改是“最小闭环”，目标是让 TCP/RDMA 共用同一个 TX machine（`m_txMachineState` 锁）且互不踩踏。

**Stop 条件（混跑特别注意）**
- `scratch/remote.cc::stop_simulation_middle()` 分别检查 RDMA 完成数和 TCP 完成数；只有两类 flow 都完成后才会提前结束。
- TCP 完成由 `PacketSink::Rx` 累计 payload 达到目标大小判定，每条 TCP flow 只计数一次。
- `Settings::cnt_finished_flows` 仍然只统计 RDMA；TCP 使用独立的 `tcp_finished_flows` 计数器。
- `FLOWGEN_STOP_TIME + simulator_extra_time` 仍是兜底硬停止时间。若某类 flow 未完成，达到该时间后仍会停止，并在日志中打印两类完成数。
...
- TCP/RDMA 共存、队列与交换机 Buffer：见 [code/14-tcp-rdma-coexist-buffer-analysis.md](code/14-tcp-rdma-coexist-buffer-analysis.md)
